...
_PATTERNS: tuple[tuple[CommandIntent, tuple[re.Pattern[str], ...]], ...] = (
    ...
    (
        ...
        (
            ...
            _full(
                r"(?:проверь|проверить)(?:\s+(?:мой|мою|последний|последнюю|текущий|текущую))?\s+(?:результат|ответ|запрос|задачу|готовность)"),
            _full(
                r"(?:что|как)\s+(?:там\s+)?(?:с\s+)?(?:моим\s+|последним\s+|текущим\s+)?(?:ответом|результатом|запросом|задачей)"),
    ...
    (
        ...
        (
            ...
            _full(
                r"(?:отмени|останови|прерви|отменить|остановить|прервать)(?:\s+мой|\s+последний|\s+текущий)?(?:\s+(?:запрос|задачу|выполнение|ответ))?"),
    ...
    (
        ...
        (
            ...
            _full(
                r"(?:начни|начать|открой|открыть)(?:\s+новый|\s+новую|\s+новое)?\s+(?:диалог|разговор|беседу|обсуждение)"),
            ...
            _full(
                r"(?:сбрось|очисти|забудь)(?:\s+текущий|\s+наш|\s+этот)?\s+(?:контекст|историю|диалог|разговор|беседу)"),
        ),
    ),
    (
        CommandIntent.EXIT,
        (
            ...
            _full(
                r"(?:заверши|завершить|закончи|закончить|прекрати|прекратить)(?:\s+наш|\s+этот)?(?:\s+(?:диалог|разговор|беседу|работу|общение))?"),
